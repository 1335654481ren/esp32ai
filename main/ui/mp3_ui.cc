...
// 定义静态成员变量
uint8_t Mp3Ui::g_sys_volume = 50; // 默认音量
lv_obj_t *Mp3Ui::music_list = nullptr; // 初始为空

// 播放指定序号的音乐
void Mp3Ui::play_index(int index)
{
...
// 设置声音处理函数
esp_err_t Mp3Ui::_audio_player_mute_fn(AUDIO_PLAYER_MUTE_SETTING setting)
{
...
esp_err_t Mp3Ui::_audio_player_write_fn(void *audio_buffer, size_t len, size_t *bytes_written, uint32_t timeout_ms)
{
    esp_err_t ret = ESP_OK;
...
// 设置采样率 播放的时候进入一次
esp_err_t Mp3Ui::_audio_player_std_clock(uint32_t rate, uint32_t bits_cfg, i2s_slot_mode_t ch)
{
    esp_err_t ret = ESP_OK;
...
// 回调函数 播放器每次动作都会进入
void Mp3Ui::_audio_player_callback(audio_player_cb_ctx_t *ctx)
{
...
// mp3播放器初始化
void Mp3Ui::mp3_player_init(void)
{
    // 获取文件信息
    file_iterator = file_iterator_new("/sccard");
...
// 按钮样式初始化
void Mp3Ui::ui_button_style_init(void)
{
...
// 播放暂停按钮 事件处理函数
void Mp3Ui::btn_play_pause_cb(lv_event_t *event)
{
...
// 上一首 下一首 按键事件处理函数
void Mp3Ui::btn_prev_next_cb(lv_event_t *event)
{
...
// 音量调节滑动条 事件处理函数
void Mp3Ui::volume_slider_cb(lv_event_t *event)
{
...
// 音乐列表 点击事件处理函数
void Mp3Ui::music_list_cb(lv_event_t *event)
{
    //lv_obj_t *label_title = (lv_obj_t *) music_list->user_data;
...
// 音乐名称加入列表
void Mp3Ui::build_file_list(lv_obj_t *music_list)
{
    lv_obj_t *label_title = (lv_obj_t *) music_list->user_data;
    ...
    for(size_t i = 0; i < 10; i++)
    {
        lvgl_port_lock(0);
        char buff[20];
        sprintf(buff, "music-%d", i);
        lv_dropdown_add_option(music_list, (const char *)buff, i); // 添加音乐名称到列表中
        lvgl_port_unlock();
    }
    lvgl_port_lock(0);
    lv_dropdown_set_selected(music_list, 0); // 选择列表中的第一个
    lv_label_set_text_static(label_title, "music-0"); // 显示list中第一个音乐的名称
    lvgl_port_unlock();
}

// 播放器界面初始化
void Mp3Ui::music_ui(void)
{
    lvgl_port_lock(0);
    ...
    /* 创建播放暂停控制按键 */
    btn_play_pause = lv_btn_create(container_);
    ...
    /* 创建上一首控制按键 */
    lv_obj_t *btn_play_prev = lv_btn_create(container_);
    ...
    /* 创建下一首控制按键 */
    lv_obj_t *btn_play_next = lv_btn_create(container_);
    ...
    lv_obj_set_style_text_font(label_next, &lv_font_montserrat_20, LV_STATE_DEFAULT);
    ...
    /* 创建声音调节滑动条 */
    volume_slider = lv_slider_create(container_);
    ...
    lv_obj_t *lab_vol_min = lv_label_create(container_);
    lv_label_set_text_static(lab_vol_min, LV_SYMBOL_VOLUME_MID);
    lv_obj_set_style_text_font(lab_vol_min, &lv_font_montserrat_18, LV_STATE_DEFAULT);
    lv_obj_align_to(lab_vol_min, volume_slider, LV_ALIGN_OUT_LEFT_MID, -10, 0);

    lv_obj_t *lab_vol_max = lv_label_create(container_);
    lv_label_set_text_static(lab_vol_max, LV_SYMBOL_VOLUME_MAX);
    lv_obj_set_style_text_font(lab_vol_max, &lv_font_montserrat_18, LV_STATE_DEFAULT);
    lv_obj_align_to(lab_vol_max, volume_slider, LV_ALIGN_OUT_RIGHT_MID, 10, 0);

    /* 创建音乐标题 */
    lv_obj_t *lab_title = lv_label_create(container_);
    ...
    /* 创建音乐列表 */ 
    music_list = lv_dropdown_create(container_);
    lv_dropdown_clear_options(music_list);
    lv_dropdown_set_options_static(music_list, "Scanning...");
    lv_obj_set_style_text_font(music_list, &lv_font_montserrat_18, LV_STATE_ANY);
...
// AI声音小一点
void Mp3Ui::ai_volume_down(void)
{
    if (g_sys_volume > 0){
        if (g_sys_volume < 5){
            g_sys_volume = 0;
